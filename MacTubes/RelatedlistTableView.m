...
@implementation RelatedlistTableView
...
- (void)awakeFromNib
{
	...
	// set row height
	float rowHeight = [self defaultFloatValue:@"optRelatedRowHeight"];
    ...
    // set click action
	[self setTarget:viewRelatedSearch];
	[self setDoubleAction:@selector(playItem:)];

	// set table column state
	[self setTableColumnState:self key:@"stateTableColumnRelatedlist"];
	...
	[self showTableColumns:tcc_ aTableView:self defaultHideCols:defaultHideCols key:@"stateTableColumnRelatedlist"];

	// set sort descriptor
	[self setArrayControllerSortDescriptor:relatedlistArrayController key:@"sortDescColumnRelatedlist"];

	// set header menu
	[[self headerView] setMenu:cmRelatedlistHeader];

	// set notification
	NSNotificationCenter *nc=[NSNotificationCenter defaultCenter];
	[nc addObserver:self selector:@selector(windowWillClose:) name:NSWindowWillCloseNotification object:[self window]];
	[nc addObserver:self selector:@selector(handleItemObjectChange:) name:VIDEO_ITEM_NOTIF_NAME_FROM_RELATED object:nil];
...
{
	[self setRowHeight:[sender intValue]];
...
-(void)windowWillClose:(NSNotification *)notification
{
	...
	// table column state
	[self saveTableColumnState:self key:@"stateTableColumnRelatedlist"];
	// save sort descriptor
	[self saveArrayControllerSortDescriptor:relatedlistArrayController key:@"sortDescColumnRelatedlist"];
        ...
        row:(int)rowIndex
{
	...
	// get record from relatedlistArrayController
	id record = [[relatedlistArrayController arrangedObjects] objectAtIndex:rowIndex];
...
- (NSString *)tableView:(NSTableView *)aTableView toolTipForCell:(NSCell *)aCell rect:(NSRectPointer)rect tableColumn:(NSTableColumn *)aTableColumn row:(int)row mouseLocation:(NSPoint)mouseLocation
{
    if( [[aTableColumn identifier] isEqualToString:@"description"] &&
		...
		id record = [[relatedlistArrayController arrangedObjects] objectAtIndex:row];
...
{
	return [[relatedlistArrayController arrangedObjects] count];
}
//------------------------------------
...
- (BOOL)tableView:(NSTableView *)tv writeRowsWithIndexes:(NSIndexSet *)rowIndexes toPasteboard:(NSPasteboard*)pboard {
	...
	NSArray *objects = [[relatedlistArrayController arrangedObjects] objectsAtIndexes:rowIndexes];
        ...
        dropOperation:(NSTableViewDropOperation)operation
{
	...
	// items
	if([items count] > 0){
		int searchType = SEARCH_WITH_URL;
		int searchSubType = SEARCH_WITH_URL_RELATED;
		// with command key -> author
		if([[[NSApplication sharedApplication] currentEvent] modifierFlags] & NSCommandKeyMask){
			searchSubType = SEARCH_WITH_URL_AUTHOR;
		}
		// set params
		NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:
									[NSNumber numberWithInt:searchType], @"searchType" ,
									[NSNumber numberWithInt:searchSubType], @"searchSubType" ,
									items, @"items" ,
									nil
								];
		// search items after delay
		SEL sel = NSSelectorFromString(@"searchWithItems:");
		[viewRelatedSearch performSelector:sel withObject:params afterDelay:0.1];
		return YES;
	}
...
{	
	...
	if (i < [self numberOfRows]){
		...
		[NSMenu popUpContextMenu:cmRelatedlist withEvent:theEvent forView:self];
		isAction = YES;
	}
